			...
			func() {
				...
				for i := 0; i < 500; i++ {
					...
					default:
						...
						headersFrame := buildValidHeadersFrameFramer(streamID, host, basePath)
						if err := framer.WriteHeaders(headersFrame); err != nil {
							return
						}
...
func buildValidHeadersFrameFramer(streamID uint32, host, path string) http2.HeadersFrameParam {
	return http2.HeadersFrameParam{
		StreamID:      streamID,
		BlockFragment: encodeHeadersHPACK(host, path),
		EndStream:     false,
		EndHeaders:    true,
	}
}

func encodeHeadersHPACK(host, path string) []byte {
...
func hpackEncoder() *hpack.Encoder {
	var buf bytes.Buffer
	return hpack.NewEncoder(&buf)
}
