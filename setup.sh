#!/bin/bash

# ----------------------------------------
# GO DOS TOOL SETUP (HTTP/2 + HTTP/1.1 ONLY)
# ----------------------------------------

# Color definitions
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
PURPLE='\033[0;35m'
CYAN='\033[0;36m'
WHITE='\033[1;37m'
NC='\033[0m' # No Color

# Separator line
SEP="━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

clear
echo -e "${CYAN}${SEP}${NC}"
echo -e "${WHITE}  DENIAL SERVICE OF GO${NC}"
echo -e "${CYAN}${SEP}${NC}"
echo

# Download main.go from GitHub
echo -e " ${YELLOW}➤${NC} ${GREEN}Downloading main.go...${NC}"
wget -q https://raw.githubusercontent.com/cyrenxxxxx/GO-DoS/refs/heads/main/main.go

# Check if download successful
if [ ! -f "main.go" ]; then
    echo -e " ${RED}✗ Error: Failed to download main.go${NC}"
    exit 1
else
    echo -e " ${GREEN}✓ Download complete${NC}"
fi

# FIX: Ensure crypto/tls is present and remove utls
echo -e " ${YELLOW}➤${NC} ${GREEN}Checking imports for crypto/tls version...${NC}"

# REMOVE utls if it exists
if grep -q 'github.com/refraction-networking/utls' main.go; then
    sed -i '/github.com\/refraction-networking\/utls/d' main.go
    sed -i '/utls "github.com\/refraction-networking\/utls"/d' main.go
    echo -e " ${GREEN}✓ Removed utls import${NC}"
fi

# ENSURE crypto/tls is present
if ! grep -q '"crypto/tls"' main.go; then
    sed -i '/import (/,/)/ s/)/\t"crypto\/tls"\n)/' main.go
    echo -e " ${GREEN}✓ Added crypto/tls import${NC}"
else
    echo -e " ${GREEN}✓ crypto/tls import already present${NC}"
fi

# REMOVE http2pkg alias if present and use standard http2
if grep -q 'http2pkg "golang.org/x/net/http2"' main.go; then
    sed -i 's/http2pkg "golang.org\/x\/net\/http2"/"golang.org\/x\/net\/http2"/g' main.go
    echo -e " ${GREEN}✓ Fixed http2 import (removed alias)${NC}"
fi

echo

# Check OS and package manager (UPDATED for modern distros)
if [ -d "/data/data/com.termux" ]; then
    echo -e " ${YELLOW}➤${NC} ${GREEN}System detected:${NC} Termux (Android)"
    PKG_MGR="pkg"
    PKG_UPDATE="pkg update"
    PKG_INSTALL="pkg install -y"
elif command -v dnf &> /dev/null && grep -qi "rhel\|fedora\|centos" /etc/os-release 2>/dev/null; then
    echo -e " ${YELLOW}➤${NC} ${GREEN}System detected:${NC} RHEL/Fedora/CentOS (modern)"
    PKG_MGR="dnf"
    PKG_UPDATE="dnf check-update"
    PKG_INSTALL="dnf install -y"
elif command -v pacman &> /dev/null; then
    echo -e " ${YELLOW}➤${NC} ${GREEN}System detected:${NC} Arch Linux"
    PKG_MGR="pacman"
    PKG_UPDATE="pacman -Sy"
    PKG_INSTALL="pacman -S --noconfirm"
elif command -v apt &> /dev/null; then
    echo -e " ${YELLOW}➤${NC} ${GREEN}System detected:${NC} Debian/Ubuntu"
    PKG_MGR="apt"
    PKG_UPDATE="apt update"
    PKG_INSTALL="apt install -y"
elif command -v zypper &> /dev/null; then
    echo -e " ${YELLOW}➤${NC} ${GREEN}System detected:${NC} openSUSE"
    PKG_MGR="zypper"
    PKG_UPDATE="zypper refresh"
    PKG_INSTALL="zypper install -y"
elif command -v apk &> /dev/null; then
    echo -e " ${YELLOW}➤${NC} ${GREEN}System detected:${NC} Alpine Linux"
    PKG_MGR="apk"
    PKG_UPDATE="apk update"
    PKG_INSTALL="apk add"
elif command -v yum &> /dev/null; then
    # Fallback for very old RHEL/CentOS 7
    echo -e " ${YELLOW}➤${NC} ${YELLOW}System detected:${NC} Old RHEL/CentOS (using yum fallback)"
    PKG_MGR="yum"
    PKG_UPDATE="yum check-update"
    PKG_INSTALL="yum install -y"
elif command -v brew &> /dev/null; then
    echo -e " ${YELLOW}➤${NC} ${GREEN}System detected:${NC} macOS"
    PKG_MGR="brew"
    PKG_UPDATE="brew update"
    PKG_INSTALL="brew install"
else
    echo -e " ${YELLOW}➤${NC} ${YELLOW}Unknown OS, assuming Linux with apt${NC}"
    PKG_MGR="apt"
    PKG_UPDATE="apt update"
    PKG_INSTALL="apt install -y"
fi

echo

# Check and install Go (with proper sudo handling)
if ! command -v go &> /dev/null; then
    echo -e " ${YELLOW}➤${NC} ${GREEN}Installing Go...${NC}"
    
    # Special handling for RHEL/Fedora - enable EPEL if needed
    if [[ "$PKG_MGR" == "dnf" ]] && ! dnf list installed epel-release &>/dev/null; then
        echo -e "   ${BLUE}→ Enabling EPEL repository...${NC}"
        dnf install -y epel-release > /dev/null 2>&1 || true
    fi
    
    # Special handling for Arch Linux
    if [[ "$PKG_MGR" == "pacman" ]]; then
        $PKG_INSTALL go > /dev/null 2>&1
    else
        # For other package managers, try with sudo if not root
        if [ "$EUID" -ne 0 ]; then
            echo -e "   ${BLUE}→ Requesting sudo privileges...${NC}"
            sudo $PKG_UPDATE > /dev/null 2>&1
            sudo $PKG_INSTALL golang > /dev/null 2>&1
        else
            $PKG_UPDATE > /dev/null 2>&1
            $PKG_INSTALL golang > /dev/null 2>&1
        fi
    fi
    
    if command -v go &> /dev/null; then
        echo -e " ${GREEN}✓ Go installed successfully${NC}"
    else
        echo -e " ${RED}✗ Failed to install Go${NC}"
        echo -e " ${YELLOW}➤${NC} Please install Go manually: https://golang.org/dl/${NC}"
        exit 1
    fi
else
    echo -e " ${GREEN}✓ Go already installed${NC}"
fi

# Check Go version
GO_VERSION=$(go version | sed -nE 's/.*go([0-9]+\.[0-9]+).*/\1/p')
if [ -z "$GO_VERSION" ]; then
    GO_VERSION="0.0"
fi
echo -e " ${YELLOW}➤${NC} ${GREEN}Go version:${NC} $GO_VERSION"

# Compare versions using awk
if ! awk -v ver="$GO_VERSION" 'BEGIN{if(ver < 1.21) exit 1; exit 0}' && [ "$GO_VERSION" != "0.0" ]; then
    echo -e " ${RED}✗ Go 1.21+ required. Current: $GO_VERSION${NC}"
    exit 1
fi

echo

# Set Go environment
export GO111MODULE=on
export CGO_ENABLED=0

# Clean old module files
echo -e " ${YELLOW}➤${NC} ${GREEN}Cleaning old module files...${NC}"
rm -rf go.mod go.sum
echo -e " ${GREEN}✓ Cleanup complete${NC}"
echo

echo -e " ${YELLOW}➤${NC} ${GREEN}Initializing Go module...${NC}"
go mod init main > /dev/null 2>&1
echo -e " ${GREEN}✓ Module initialized${NC}"

echo

# Download dependencies (specific versions) - REMOVED utls
echo -e " ${YELLOW}➤${NC} ${GREEN}Downloading dependencies...${NC}"
go get golang.org/x/net@v0.24.0 > /dev/null 2>&1
go get golang.org/x/crypto@v0.22.0 > /dev/null 2>&1
go get golang.org/x/sys@v0.19.0 > /dev/null 2>&1
echo -e " ${GREEN}✓ Dependencies downloaded${NC}"

echo

# Tidy up
echo -e " ${YELLOW}➤${NC} ${GREEN}Running go mod tidy...${NC}"
go mod tidy > /dev/null 2>&1
echo -e " ${GREEN}✓ Tidy complete${NC}"

echo
echo -e "${CYAN}${SEP}${NC}"
echo -e "${WHITE}  COMPILATION PROCESS${NC}"
echo -e "${CYAN}${SEP}${NC}"
echo

# Compile with retry mechanism
echo -e " ${YELLOW}➤${NC} ${GREEN}Compiling...${NC}"
MAX_RETRIES=3
RETRY_COUNT=0

while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
    echo -ne " ${YELLOW}Attempt $((RETRY_COUNT+1))/$MAX_RETRIES...${NC} "
    
    # Capture build output
    BUILD_OUTPUT=$(go build -o main main.go 2>&1)
    BUILD_EXIT_CODE=$?
    
    if [ $BUILD_EXIT_CODE -eq 0 ]; then
        chmod +x main
        echo -e "${GREEN}✓ Success!${NC}"
        echo
        
        # Cleanup
        echo -e " ${YELLOW}➤${NC} ${GREEN}Cleaning up...${NC}"
        rm -f main.go go.mod go.sum
        echo -e " ${GREEN}✓ Cleanup complete${NC}"        
        echo
        echo -e "${CYAN}${SEP}${NC}"
        echo -e "${WHITE}  SETUP COMPLETE${NC}"
        echo -e "${CYAN}${SEP}${NC}"
        echo
        echo -e " ${GREEN}►${NC} Run: ${YELLOW}./main <target> <seconds> <GET|POST|HEAD|SLOW> [proxy]${NC}"
        echo
        exit 0
    else
        echo -e "${RED}✗ Failed${NC}"
        if [ -n "$BUILD_OUTPUT" ]; then
            echo -e " ${RED}Error details:${NC}"
            while IFS= read -r line; do
                echo -e "   ${RED}→${NC} $line"
            done <<< "$BUILD_OUTPUT"
        fi
    fi
    
    RETRY_COUNT=$((RETRY_COUNT+1))
    
    if [ $RETRY_COUNT -lt $MAX_RETRIES ]; then
        echo -e " ${YELLOW}⟲ Retrying... (Attempt $((RETRY_COUNT+1))/$MAX_RETRIES)${NC}"
        
        # Fix common issues on each retry
        case $RETRY_COUNT in
            1)
                echo -e "   ${BLUE}→ Running go mod download...${NC}"
                go mod download > /dev/null 2>&1
                ;;
            2)
                echo -e "   ${BLUE}→ Cleaning module cache...${NC}"
                go clean -modcache > /dev/null 2>&1
                go get -u ./... > /dev/null 2>&1
                go mod tidy -go=1.21 > /dev/null 2>&1
                ;;
        esac
        echo
    fi
done

# If all retries failed, try with -mod=mod
echo -e " ${YELLOW}➤${NC} ${YELLOW}Last resort: Using -mod=mod flag...${NC}"
LAST_RESORT_OUTPUT=$(go build -mod=mod -o main main.go 2>&1)
LAST_RESORT_CODE=$?

if [ $LAST_RESORT_CODE -eq 0 ]; then
    chmod +x main
    echo -e " ${GREEN}✓ Compilation successful with -mod=mod!${NC}"
    echo
    echo -e " ${YELLOW}➤${NC} ${GREEN}Cleaning up...${NC}"
    rm -f main.go go.mod go.sum
    echo -e " ${GREEN}✓ Cleanup complete${NC}"
    
    echo
    echo -e "${CYAN}${SEP}${NC}"
    echo -e "${WHITE}  SETUP COMPLETE${NC}"
    echo -e "${CYAN}${SEP}${NC}"
    echo
    echo -e " ${GREEN}►${NC} Run: ${YELLOW}./main <target> <seconds> <GET|POST|HEAD|SLOW> [proxy]${NC}"
    echo
    exit 0
else
    if [ -n "$LAST_RESORT_OUTPUT" ]; then
        echo -e " ${RED}Final error:${NC}"
        while IFS= read -r line; do
            echo -e "   ${RED}→${NC} $line"
        done <<< "$LAST_RESORT_OUTPUT"
    fi
fi

echo
echo -e "${CYAN}${SEP}${NC}"
echo -e "${WHITE}  SETUP FAILED${NC}"
echo -e "${CYAN}${SEP}${NC}"
echo

# If everything failed
echo -e " ${RED}✗ All compilation attempts failed!${NC}"
echo
echo -e " ${YELLOW}➤${NC} ${GREEN}Try running these commands manually:${NC}"
echo
echo -e "   ${BLUE}1.${NC} go mod init main"
echo -e "   ${BLUE}2.${NC} go get golang.org/x/net@v0.24.0"
echo -e "   ${BLUE}3.${NC} go get golang.org/x/crypto@v0.22.0"
echo -e "   ${BLUE}4.${NC} go get golang.org/x/sys@v0.19.0"
echo -e "   ${BLUE}5.${NC} go mod tidy"
echo -e "   ${BLUE}6.${NC} go build -o main main.go"
echo

rm -f main.go
exit 1