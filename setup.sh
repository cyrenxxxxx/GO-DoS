...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
    ...
    if [ -f "main" ]; then
        ...
        echo "Done! Run: ./main <target> <seconds> <GET|POST|HEAD|SLOW|WS> [proxy]"
        exit 0
    fi
    
    RETRY_COUNT=$((RETRY_COUNT+1))
    
    if [ $RETRY_COUNT -lt $MAX_RETRIES ]; then        echo "⚠️ Compilation failed, retrying... (Attempt $((RETRY_COUNT+1))/$MAX_RETRIES)"
...
echo "  go get github.com/gorilla/websocket@latest"
echo "  go mod tidy"
