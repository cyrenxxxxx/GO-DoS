...
if [ -d "/data/data/com.termux" ]; then
    ...
    PKG_MGR="pacman -S"
elif command -v yum &> /dev/null; then    echo -e " ${YELLOW}➤${NC} ${GREEN}System detected:${NC} CentOS/RHEL"
    PKG_MGR="yum"
...
# Download dependencies (HTTP/2 + uTLS for real browser fingerprints)
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
    ...
    # Capture build output (success = empty, error = message)
...
if [ $LAST_RESORT_CODE -eq 0 ]; then
    chmod +x main    echo -e " ${GREEN}✓ Compilation successful with -mod=mod!${NC}"
    echo
    echo -e " ${YELLOW}➤${NC} ${GREEN}Cleaning up...${NC}"
    rm -f main.go go.mod go.sum
    echo -e " ${GREEN}✓ Cleanup complete${NC}"
...
echo -e "   ${BLUE}5.${NC} go get github.com/refraction-networking/utls@latest"
